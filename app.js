...
const pageInputWrap = document.getElementById("pageInputWrap");
const actualPagesInput = document.getElementById("actualPagesInput");
...
function renderCard() {
  ...
  if (!card) {
    ...
    actualPagesInput.value = "";
  } else {
    const goalLine =
      card.goalType === "fixed_pages"
        ? `<span>${card.goalValue} Seiten</span>`
        : `<span>Seiten selbst eintragen</span>`;
    activeCard.innerHTML = `
    ...
    `;
    pageInputWrap.classList.toggle("hidden", card.goalType === "fixed_pages");
...
document.getElementById("completeButton").addEventListener("click", async () => {
  try {
    const payload = {};
    if (state.user?.currentCard?.goalType !== "fixed_pages") {
      payload.actualPages = actualPagesInput.value;
    }
    const data = await api("/api/cards/complete", { method: "POST", body: JSON.stringify(payload) });
